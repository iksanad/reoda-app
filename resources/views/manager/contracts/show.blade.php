    ...
    <div class="lg:col-span-2 space-y-5">
        ...
        <div class="rounded-2xl border border-gray-200 bg-white shadow-sm" x-data="{ openInvoice: false }">
            ...
            <div x-show="openInvoice" x-transition style="display:none"
                ...
                <div class="w-full max-w-lg rounded-2xl bg-white shadow-2xl overflow-y-auto max-h-[90vh]" x-data="{ invType: 'rent', iplDefault: {{ $contract->unit->property->ipl_amount ?? 0 }}, rentDefault: {{ $contract->rent_amount ?? 0 }} }">
                    ...
                    <form action="{{ route('manager.contracts.invoices.store', $contract) }}" method="POST" class="p-6 space-y-4">
                        ...
                        <div>
                            ...
                            <p x-show="invType === 'rent' && rentDefault > 0" class="mt-1 text-xs text-blue-600">
                                Sesuai harga sewa kontrak: Rp {{ number_format($contract->rent_amount ?? 0, 0, ',', '.') }}
                            </p>
                        </div>
                        <div>
